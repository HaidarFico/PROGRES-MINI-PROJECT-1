    ...
    try:
        ...
        if sock in socket_map and socket_map[sock] and sock.getpeername():
            ...
            if (isHTTPMessageComplete(recvBuffers.get(sock, b''))):
                logClientRequest(recvBuffers.get(sock, b'').getpeername(), serverHost)
...
def handleServerResponse(serverSock: socket, clientSock: socket, currentBuffer: bytes):
    ...
    if isBodyParsed and isHeaderParsed:
        ...
        logServerResponse(clientSock.getpeername(), serverSock.getpeername(), parseHTTPStatus(parsedHttpHeaders), len(parsedHttpHeaders) + len(parsedHttpBody))
        if clientSock:
            clientSock.sendall(parsedHttpHeaders + parsedHttpBody)
        else:
            close_relay(serverSock)
    else:
        close_relay(serverSock)

def parseHTTPStatus(headerBytes: bytes):
    try:
        ...
        headerStatusLine = headerLines[0].strip()
        if not headerStatusLine.startswith('HTTP/'):
            return "STATUS NOT FOUND"
        
        headerStatusParts = headerStatusLine.split(' ', 2)
        if headerStatusLine < 2:
            return "STATUS NOT FOUND"
        
        statusCode = int(headerStatusParts[1])
...
def logClientRequest(clientIP, siteAccessed):
    try:
        with open('loggersLog.txt', "a") as f:
            currentTime = datetime.now()
            f.write(f'{currentTime};REQUEST;{clientIP};{siteAccessed},GET\n')
            f.close()
    except:
        with open('loggersLog.txt', "w") as f:
            f.close()
            logClientRequest(clientIP, siteAccessed)
